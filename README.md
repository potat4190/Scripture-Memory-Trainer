...
**Phase 3 complete — the pure logic is pinned by every fixture the workbook
supplies, and the whole flow now runs over HTTP.** Seed, queue, check, review,
travel sixty days, export and restore are all drivable from `/docs` with no
frontend at all. See [docs/BUILD-CHECKLIST.md](docs/BUILD-CHECKLIST.md) for the
plan.
...
| 4 | Static HTML / Alpine.js frontend, multi-script rendering | next |
...
`uv run pytest` — **301 passed, 3 xfailed**, and it writes
...
src/scripture_memory_trainer/
  ...
  api.py           eight endpoints and nothing else
...
uv run pytest                 # 301 passed, 3 xfailed (the documented workbook errors)
