...
**Phase 2 complete — the pure logic exists and is pinned by a test suite that
runs every fixture the workbook supplies. Phase 3 is under way:** the schema,
its migration, the seed loader and the FastAPI app are in and tested; the seven
`/api` endpoints are the remaining work. See
...
| 3 | FastAPI backend + SQLModel + Alembic | **in progress** — schema, migration, seed, app skeleton done; endpoints next |
...
`uv run pytest` — **267 passed, 3 xfailed**, and it writes
...
tests/test_storage.py                save/load round trips for the JSON state file
...
tests/test_api.py                    app boots, /docs renders, DB dependency injectable
...
uv run pytest                 # 267 passed, 3 xfailed (the documented workbook errors)
...
```bash
uv run alembic upgrade head   # creates the tables (SQLite by default)
uv run python -c "from sqlmodel import Session;   from scripture_memory_trainer.database import engine;   from scripture_memory_trainer.seed import seed_cards;   print(seed_cards(Session(engine)))"          # idempotent: safe to re-run
uv run uvicorn scripture_memory_trainer.api:app --reload
```

Then open <http://127.0.0.1:8000/docs>. `DATABASE_URL` overrides the database;
