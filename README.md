...
runs every fixture the workbook supplies.** No database, network, or framework is
involved yet; that is the point of these phases. See
...
| 3 | FastAPI backend + SQLModel + Alembic | next |
...
`uv run pytest` — **221 passed, 3 xfailed**, and it writes
...
uv run pytest                 # 221 passed, 3 xfailed (the documented workbook errors)
